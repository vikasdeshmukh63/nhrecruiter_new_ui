...
const STATUS_OPTIONS = [
  ...
  { value: 'sent request', label: 'Sent Request', statusValue: 1 },
  { value: 'cancelled request', label: 'Cancelled Request', statusValue: 2 },
  { value: 'accepted', label: 'Accepted', statusValue: 3 },
  { value: 'resend', label: 'Resend', statusValue: 4 },
  { value: 'interview scheduled', label: 'Interview Scheduled', statusValue: 5 },
];

// table heads
const TABLE_HEAD = [
  { id: 'candidateName', label: 'Candidate Name / Schedule #' },
...
export default function SchedulesListView() {
  ...
  const { schedules, itemCount, error } = useSelector((state) => state.schedules);
  ...
  useEffect(() => {
    if (selectedJobPost) {
      dispatch(fetchScheduleseList(table.page, table.rowsPerPage, status, selectedJobPost.id_str));
    } else {
      dispatch(fetchScheduleseList(table.page, table.rowsPerPage, status));
    }
    ...
    <DashboardContent maxWidth="xl">
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            { name: 'Application', href: paths.application.schedules },
            { name: 'Schedule', href: paths.application.schedules },
            { name: 'List' },
          ]}
          sx={{
            mb: { xs: 3, md: 5 },
