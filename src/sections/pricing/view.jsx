...
import { fetchPlansFeaturesList, fetchPlansCategoriesList } from 'src/redux/slices/subscription';
...
export default function PricingView() {
  ...
  const { planData, planCategories } = useSelector((state) => state.subscription);

  useEffect(() => {
    dispatch(fetchPlansFeaturesList(0, 5));
    dispatch(fetchPlansCategoriesList(0, 5));
  }, [dispatch]);

  const handlePriceChange=(e)=>{
    setPriceType((prev)=> prev === "monthly" ? "yearly" : "monthly")
  }
    ...
    >
      ...
      <Box sx={{ mt: 9, mb: 5, position: 'relative' }}>
        <Stack direction="row" alignItems="center" justifyContent="center">
          ...
          <Switch sx={{ mx: 1 }} value={priceType} onChange={handlePriceChange}/>
      ...
      >
        {planData.map((card, index) => (
          <PricingCard key={card.subscription} card={card} planCategories={planCategories} index={index} priceType={priceType}/>
        ))}
      </Box>
    </Container>
  );
