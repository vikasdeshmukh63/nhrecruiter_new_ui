...
const AddEditEducationDetailsModal = ({ isOpenModal, educationData, type }) => {
  ...
  const { error: formErr, editCandidateId, collegeData } = useSelector((state) => state.jobpost);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      if (type === CONSTANTS.EDIT) {
        ...
        await dispatch(editEducationDetails(editCandidateId, payload));
      } else {
        await dispatch(addEducationDetails(editCandidateId, payload));
      }
      isSubmitted.onTrue();
    } catch (err) {
      console.log(err);
  ...
  useEffect(() => {
    ...
    if (isSubmitted.value && !formErr) {
      ...
      dispatch(getCandidateWorkEducationDetails(editCandidateId));
      isOpenModal.onFalse();
      isSubmitted.onFalse();
    }
