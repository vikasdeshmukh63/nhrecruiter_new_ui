...
const AddEditSocialProfilesDetailsModal = ({ isOpenModal, socialProfileData, type }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      if (type === CONSTANTS.EDIT) {
        ...
        await dispatch(editSocialProfileDetails(editCandidateId, payload));
      } else {
        await dispatch(addSocialProfilesDetails(editCandidateId, payload));
      }
      isSubmitted.onTrue();
    } catch (err) {
      console.log(err);
  ...
  useEffect(() => {
    ...
    if (isSubmitted.value && !formErr) {
      ...
      dispatch(getSocialProfilesDetails(editCandidateId));
      isOpenModal.onFalse();
      isSubmitted.onFalse();
    }
