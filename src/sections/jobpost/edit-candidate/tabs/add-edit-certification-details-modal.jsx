...
const AddEditCertificationDetailsModal = ({ isOpenModal, certificationData, type }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      if (type === CONSTANTS.EDIT) {
        ...
        await dispatch(editCertificationDetails(editCandidateId, payload));
      } else {
        await dispatch(addCertificationsDetails(editCandidateId, payload));
      }
      isSubmitted.onTrue();
    } catch (err) {
      console.log(err);
  ...
  useEffect(() => {
    ...
    if (isSubmitted.value && !formErr) {
      ...
      dispatch(getCandidateCertificationDetails(editCandidateId));
      isOpenModal.onFalse();
      isSubmitted.onFalse();
    }
